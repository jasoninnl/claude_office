...
import { X, Users, Crown, HelpCircle } from "lucide-react";
...
interface Props {
  ...
  teams: Team[];           // all teams on this floor (from scenario allocations)
  onElementsChange: () => void;
  drawMode: "none" | "desk" | "office"; // controlled by parent toolbar
  onDrawModeChange: (m: "none" | "desk" | "office") => void;
}
...
function teamColor(teamId: number | null, teams: Team[], alpha: number): string {
  if (!teamId) return `rgba(75,85,99,${alpha})`;   // gray-600
...
export default function PdfFloorPlanCanvas({
  ...
  // Convert a mouse event to normalised SVG coordinates (0–1)
  ...
  // Click on the SVG background — place a new element in draw mode
        ...
        const ids = new Set(
          elements
            .filter((el) => {
              ...
              return cx >= lasso.x && cx <= lasso.x + lasso.w
                  && cy >= lasso.y && cy <= lasso.y + lasso.h;
            })
            .map((el) => el.id)
        );
        setSelected(ids);
  ...
  const reclassify = async (id: number, type: "desk" | "office") => {
    setSaving(true);
    try {
  ...
  const popoverElement = popover
    ? elements.find((e) => e.id === popover.elementId) ?? null
    : null;
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500 flex-wrap">
        <span>
          <span className="text-blue-400 font-medium">{desks.length}</span> desks detected
          {assignedDesks > 0 && <span className="text-gray-600"> · {assignedDesks} assigned</span>}
        </span>
        <span>
          <span className="text-purple-400 font-medium">{offices.length}</span> offices detected
        ...
        <span className="ml-auto text-gray-600 italic">
          {drawMode === "none"
            ...
            : `Click to place ${drawMode} · Click toolbar button again to cancel`}
        </span>
      </div>
      ...
      >
        ...
        {/* SVG overlay — viewBox 0 0 1 1, preserveAspectRatio none */}
        <svg
          ref={svgRef}
          className="absolute inset-0 w-full h-full"
          viewBox="0 0 1 1"
          ...
          {offices.map((el) => {
            ...
            return (
              ...
              >
                ...
                {el.is_lead_office && (
                  <>
                    {/* Crown icon as SVG path */}
                    <text
                      x={cx} y={el.ny + el.nh * 0.38}
                      textAnchor="middle" dominantBaseline="middle"
                      fontSize={fontSize * 1.1}
                      fill="white" style={{ filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.8))" }}
                    >
                      👑
                    </text>
                    <text
                      x={cx} y={el.ny + el.nh * 0.72}
                      textAnchor="middle" dominantBaseline="middle"
                      fontSize={Math.min(fontSize, 0.018)}
                ...
                {!el.is_lead_office && el.team_id && (
                  <text
                    x={cx} y={cy}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize={Math.min(fontSize, 0.02)}
                    fill="white" fontWeight="600"
                    style={{ filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.9))" }}
                  >
        ...
        {/* Bulk-assign bar when multiple selected and no popover */}
        {selected.size > 1 && !popover && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-gray-900 border border-gray-600 rounded-xl px-3 py-2 flex items-center gap-2 shadow-xl">
...
interface PopoverProps {
  ...
  onDelete: () => void;
  onReclassify: (type: "desk" | "office") => void;
  onClose: () => void;
}
...
function ElementPopover({ element, teams, screenX, screenY, containerRef, onAssign, onDelete, onReclassify, onClose }: PopoverProps) {
  ...
  const left = rect ? screenX - rect.left : 0;
  const top = rect ? screenY - rect.top : 0;
  const currentTeam = teams.find((t) => t.id === element.team_id);

  return (
    <div
      className="absolute z-20 bg-gray-900 border border-gray-600 rounded-xl shadow-2xl p-3 w-52"
      style={{ left: `${left}px`, top: `${Math.max(8, top - 10)}px`, transform: "translate(-50%, -100%)" }}
      onClick={(e) => e.stopPropagation()}
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-gray-300 capitalize">
          {element.element_type === "office" ? "🏠 Office" : "🪑 Desk"}
      ...
      <div className="space-y-1">
        ...
        <div className="border-t border-gray-700 pt-1.5 mt-1.5 flex gap-2">
          <button
            onClick={() => onReclassify(element.element_type === "desk" ? "office" : "desk")}
            className="flex-1 text-xs text-gray-400 hover:text-white bg-gray-800 hover:bg-gray-700 rounded px-2 py-1"
          >
            → {element.element_type === "desk" ? "Mark as office" : "Mark as desk"}
          </button>
          <button
            onClick={onDelete}
